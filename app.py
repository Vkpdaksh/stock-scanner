...
# -------------------------------------------------------------
# 5. DATA ENGINE & SCANNER (Fetched early for price references)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 6B. ACTIVE POSITIONS WITH LIVE P&L & SQUARE-OFF BUTTONS
# -------------------------------------------------------------
if open_trades:
    st.markdown("#### ⚡ Active Open Positions (Live Tracking)")
    for idx, trade in enumerate(open_trades):
        asset_name = trade.get('asset')
        # Find current live price from records or fetch quickly
        ...
        t_col1, t_col2, t_col3, t_col4, t_col5 = st.columns([2, 1.8, 1.8, 1.5, 1.2])
        with t_col1:
            st.write(f"**{asset_name}** ({t_type})")
            st.caption(f"Qty: {qty} | Entry: ₹{entry_price}")
        with t_col2:
            st.markdown(f"**LTP:** ₹{current_ltp}")
        with t_col3:
            st.markdown(f"**P&L:** <span style='color:{pnl_color}; font-weight:bold;'>₹{live_pnl:+,.2f}</span>", unsafe_allow_html=True)
        with t_col4:
            st.caption(f"SL: ₹{trade.get('sl')} | TP: ₹{trade.get('tp1')}")
        with t_col5:
            if st.button(f"🔴 Exit", key=f"sq_off_{idx}"):
                ...
                # Recalculate balance properly on exit
                save_json(PAPER_TRADES_FILE, paper_data)
