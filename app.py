...
# -------------------------------------------------------------
# 12. DUAL EXECUTION DESK WITH SL & TP INPUTS
# -------------------------------------------------------------
...
with sl_tp_col1:
    custom_sl = st.number_input("Stop Loss (SL ₹):", min_value=0.01, value=float(default_sl), step=0.05, format="%.2f", help="Jab market is level par aayegi toh position auto exit ho jayegi")
with sl_tp_col2:
    custom_tp = st.number_input("Target Price (TP ₹):", min_value=0.01, value=float(default_tp), step=0.05, format="%.2f", help="Jab market target par pahuchegi toh profit auto book ho jayega")
