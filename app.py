...
# Telegram dispatch function via standard urllib
...
# Auto send to Telegram after 3:35 PM IST once per day
if (ist_now.hour == 15 and ist_now.minute >= 35) or (ist_now.hour > 15):
    if eod_tracker.get("last_sent_date") != today_date_str and tot_alerts_today > 0:
        if send_telegram_msg(build_eod_message()):
...
# Real-Time EOD Summary Card on Terminal Desk
with st.expander("📊 Today's EOD Report & Telegram Dispatch", expanded=False):
    ...
    if st.button("📤 Send Accurate EOD Report to Telegram Now"):
        if send_telegram_msg(build_eod_message()):
            st.success("Accurate EOD Report successfully sent to Telegram!")
        else:
            st.warning("Telegram Bot Token / Chat ID check karein.")

# -------------------------------------------------------------
# 8B. ACTIVE POSITIONS WITH VISUAL PROGRESS & MANUAL EXIT
# -------------------------------------------------------------
if open_trades:
