    ...
    for ticker in tickers:
        try:
            ...
            if is_breakout:
                ...
                if (rvol >= 2.2 or is_special) and (c_close > ema50) and (rsi >= 58):
                    grade = "Grade A+ (Sniper)"
                    has_sniper_alert = True
                elif (rvol >= 1.4 or is_special) and (rsi >= 52):
                    grade = "Grade A (Inst.)"
                else:
                    grade = "Grade B (Scalp)"
            elif is_breakdown:
                ...
                if (rvol >= 2.2 or is_special) and (c_close < ema50) and (rsi <= 42):
                    grade = "Grade A+ (Sniper)"
                    has_sniper_alert = True
                elif (rvol >= 1.4 or is_special) and (rsi <= 48):
                    grade = "Grade A (Inst.)"
                else:
                    grade = "Grade B (Scalp)"
            else:
...
for trade in all_trades:
    if trade.get("status") == "OPEN":
        ...
        c_ltp = get_live_price_for_asset(a_name)
        if c_ltp:
            ...
            if sl_hit or tp_hit:
                ...
                trade["exit_time"] = get_ist_now().strftime("%Y-%m-%d %H:%M")
...
# -------------------------------------------------------------
# 7. VIRTUAL PAPER TRADING PORTFOLIO & LIVE P&L ENGINE
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 7B. ACTIVE POSITIONS WITH VISUAL PROGRESS & MANUAL EXIT
# -------------------------------------------------------------
if open_trades:
...
if selected_item:
    ...
    if "BUY" in sig_type:
        ...
        if default_ltp < vwap_val:
            st.error("🔴 **Setup Invalidated:** Price VWAP ke niche gir chuka hai. Is trade mein entry na karein.")
        elif dist_pct > 0.4:
            st.warning(f"🟡 **Overextended (+{dist_pct:.2f}%):** Price bhaag chuka hai. Chasing na karein, pullback ya re-test ka intezaar karein.")
        else:
            st.success(f"🟢 **Safe Entry Zone (+{dist_pct:.2f}%):** Price breakout level aur VWAP ke paas hai. Tight Stop Loss ke sath entry valid hai.")
    elif "SELL" in sig_type:
        dist_pct = ((brk_lvl - default_ltp) / brk_lvl) * 100
        if default_ltp > vwap_val:
            st.error("🔴 **Setup Invalidated:** Price wapas VWAP ke upar chala gaya hai. Breakdown fail ho chuka hai.")
        elif dist_pct > 0.4:
            st.warning(f"🟡 **Overextended (-{dist_pct:.2f}%):** Sell pressure me price bohot niche nikal gaya hai. Retest ka wait karein.")
        else:
            st.success(f"🟢 **Safe Entry Zone (-{dist_pct:.2f}%):** Support breakdown zone ke paas hai. Short entry valid hai.")
...
# Row 2: Dedicated Stop Loss and Target inputs
