...
# -------------------------------------------------------------
# WATCHLIST REGISTRY WITH EXPLICIT MARKET LABELS
# -------------------------------------------------------------
MARKET_CATEGORIES = {
    "🇮🇳 INDIAN EQUITIES (NSE)": [
...
# -------------------------------------------------------------
# BATCH SCANNER WITH STRICT REPEAT BLOCK & 1:2 RRR
# -------------------------------------------------------------
def run_batch_market_scan(sent_cache, active_trades, ist_now):
    ...
    # DUPLICATE BLOCK: Already sent or active trades completely excluded
    scan_candidates = [t for t in all_tickers if t not in sent_cache and t not in active_trades]
    
    # Calculate current 15m entry window
        ...
        try:
            ...
            for ticker in batch:
                try:
                    ...
                    # Volatility and ATR for 1:2 RRR
                    ...
                    # STRICT 1:2 RRR FORMULA
                    sl_dist = 1.0 * atr
                    sl = round(c_close - sl_dist, 4 if is_forex_or_comm else 2)
                    tp = round(c_close + (2.0 * sl_dist), 4 if is_forex_or_comm else 2)  # Exact 1:2
                    ...
                    msg = (
                        ...
                        f"🎯 *Target (Fixed 1:2):* {currency}{tp}\n\n"
                    ...
                    # IMMEDIATELY LOCK TICKER TO PREVENT REPEATS
                    ...
                    print(f"Alert successfully sent for {display_name} [{market_tag}]")
                except Exception:
                    continue
...
# -------------------------------------------------------------
# MAIN ENGINE WITH 11:00 PM CUTOFF & PERSISTENT DEDUPLICATION
# -------------------------------------------------------------
if __name__ == "__main__":
    ...
    if ist_now.hour >= 23 or ist_now.hour < 8:
        print(f"[{ist_now.strftime('%H:%M IST')}] Market hours cutoff active (11:00 PM - 8:00 AM). Exiting clean.")
        exit(0)

    # Load cache and enforce daily reset
